...
import TableData from './../components/table/software_patching/table-data'
...
let SoftwarePatching = React.createClass({
  ...
  componentDidMount: function() {
    var self = this;
    $.ajax({
      url: Constants.api_base_url + Constants.api_version + '/findings/software_patching/' + localStorage.analysis_id,
      success: (data) => {
    ...
    onSubmit(e) {
      $.ajax({
      type: 'PUT',
      url: Constants.api_base_url + Constants.api_version + '/analyses/software_patching/' + localStorage.analysis_id + '/' + localStorage.unique_key,
      crossDomain: true,
      dataType: 'json',
      contentType: 'application/json',
      success: (data) => {
    ...
    render: function(){
      return (
        ...
        <div>
        <h1>Software Patching</h1>
      <input type="text" value={this.state.search_term} onFocus={this.gainFocus} onChange={this.handleSearch} />
