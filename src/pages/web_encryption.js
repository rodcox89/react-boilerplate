...
let WebEncryption = React.createClass({
  ...
  componentDidMount: function() {
    var self = this;
    $.ajax({
      url: 'http://0.0.0.0:5000/v1/findings/web_encryption/'+localStorage.analysis_id,
      success: (data) => {
        console.log(data)
    ...
    onSubmit(e) {
      ...
      $.ajax({
      type: 'GET',
      url: 'http://0.0.0.0:5000/v1/update_analyses/web_encryption/'+localStorage.analysis_id+'/'+localStorage.unique_key,
      crossDomain: true,
      dataType: 'json',
      contentType: 'application/json',
      success: (data) => {
    ...
    render: function(){
      return (
        ...
        { this.state.loaded & this.state.has_results ?
          ...
          { !this.has_results ?
            <div className="container">
              <p className="error-text">Sorry! There are no web encryption findings for analysis_id: { localStorage.analysis_id }</p>
            </div>
            :null}
          </div>
